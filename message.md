...
search.md가 만든 "주제 후보 5개"를 텔레그램으로 전송하고,
...
- 사용자가 "1"~"5" 중 하나로 답장 (또는 수정 요청 텍스트)
- 숫자 응답이면 → 해당 후보를 선택된 주제로 확정
- 숫자가 아니면 → 자유 텍스트를 요청/피드백으로 간주하고, 그 피드백 반영한 새 후보를 다시 5개 생성해 재전송 (최대 2회 재시도, 이후엔 사람이 직접 개입)

### 3-1. 확정된 주제 기록 (다음 실행에서 중복 방지)
주제가 확정되면, 저장소의 `used_topics.txt`에 아래 형식으로 한 줄 추가한다 (파일이 없으면 새로 생성):
```
{날짜} | {확정된 주제}
```
search.md가 다음 실행 때 이 파일을 읽어서 같은 주제가 다시 후보로 나오지 않게 한다.
